...
local parserVersion = "2016.07.11.2d3"
...
2016.07.11.2d3  william motley  Completely rewritten.  Much better IPv6 validation
2016.03.21.1  william motley  Trunacte at forward slash
2015.09.01.1  william motley  Use string.gmatch instead of looping string.find
2014.12.11.1  william motley  Return nil for empty strings
2013.02.26.1  william motley  Detect malformed IPv6
2012.10.04.1  william motley  Use constructor to initialize table "dot"
2012.10.02.1  william motley  Localize global functions
2012.08.13.1  william motley  Use "alias.ip" not "alias.ipv4"
2012.07.23.1  william motley  Initial development
...
2013.11.07.1  william motley  Initial development
...
2016.04.07.1  william motley  Use string.match
2015.09.01.1  william motley  Complete rewrite
2014.12.11.1  william motley  Return nil for empty strings
2013.02.27.1  william motley  If length of fullpath is 0 then don't bother
2012.10.04.1  william motley  Bugfix: set lastPosition to nil before extracting extension
2012.10.02.1  william motley  Localize global functions
2012.07.28.1  william motley  Initial development
...
function extractPathElements(fullpath)
    ...
    if string.find(fullpath, "^.*[/\\]") then
        dir, file = string.match(fullpath, "^(.*[/\\])(.*)")
    else
        file = fullpath
    end
    dir = (dir and string.len(dir) ~= 0 and dir) or nil
    file = (file and string.len(file) ~= 0 and file) or nil
    if file and string.find(file, "^.*%.") then
        ext = string.match(file, "^.*%.(.*)$")
    end
    ext = (ext and string.len(ext) ~= 0 and ext) or nil
    return dir, file, ext
end
...
2016.01.04.1  william motley  accomodate ; as a querystring separator
2015.09.01.1  william motley  truncate at first unprintable
                                      extractPathElements() returns nil for empty string, no need to check here
2015.03.19.1  william motley  Run though urldecode() after querystring is extracted
2014.12.11.1  william motley  Return nil for empty strings
2014.11.12.1  william motley  Make sure URI doesn't appear too far into URL
2014.02.14.1  william motley  Rework (again) to accomodate colons in filenames (*sigh*)
2013.10.29.3  william motley  Rework separation of host from path
2013.10.17.1  william motley  Bugfix: urlQuery -> urlQuerystring
2013.06.14.1  william motley  Initial development
...
function extractUrlElements(url)
    ...
    local stringFind, stringSub, stringLen = string.find, string.sub, string.len
    local urlHost, urlDirectory, urlFilename, urlExtension, urlQuerystring
    -- truncate at unprintable, whitespace or quotation
    local found = stringFind(url, "[%c%s\'\"]", 1)
    if found then
        url = stringSub(url, 1, found - 1)
    end
    if stringLen(url) == 0 then
        return
    end
    local queryPos = stringFind(url, "[?&=;]")
    if queryPos then
        urlQuerystring = stringSub(url, queryPos + 1, -1)
        url = stringSub(url, 1, queryPos - 1)
    end
    local uriPos = stringFind(url, "://")
    if uriPos and uriPos <= 6 then
        url = stringSub(url, uriPos + 3, -1)
    end
    url = urldecode(url)
    local dirPos = stringFind(url, "/")
    if dirPos then
        urlDirectory, urlFilename, urlExtension = extractPathElements(stringSub(url, dirPos, -1))
        urlHost = stringSub(url, 1, dirPos - 1)
    else
        urlHost = url
    end
    urlHost = (urlHost and string.len(urlHost) > 0 and urlHost) or nil
    urlQuerystring = (urlQuerystring and string.len(urlQuerystring) > 0 and urlQuerystring) or nil
    return urlHost, urlDirectory, urlFilename, urlExtension, urlQuerystring
end

--[=[               -----------------------------------
                   EXTRACT INTEGER VALUES FROM STRINGS
                   -----------------------------------

2016.05.20.1  william motley  Split from readASNlength() and decodeASN()

Description todo...
--]=]

function struint16(inString, position)
    if inString and position and #inString >= position + 1 then
        -- doesn't support little-endian or multiple return values (todo?)
        local tempByte1, tempByte2 = string.byte(inString, position, position + 1)
        if tempByte1 and tempByte2 then
            return (tempByte1 * 256) + tempByte2
        end
    end
end

function struint24(inString, position)
    if inString and position and #inString >= position + 2 then
        local tempByte1, tempByte2, tempByte3 = string.byte(inString, position, position + 2)
        if tempByte1 and tempByte2 and tempByte3 then
            return (((tempByte1 * 256) + tempByte2) * 256) + tempByte3
        end
    end
end
    
function struint32(inString, position)
    if inString and position and #inString >= position + 3 then
        -- doesn't support little-endian or multiple return values (todo?)
        local tempByte1, tempByte2, tempByte3, tempByte4 = string.byte(inString, position, position + 3)
        if tempByte1 and tempByte2 and tempByte3 and tempByte4 then
            return (((((tempByte1 * 256) + tempByte2) * 256) + tempByte3) * 256) + tempByte4
        end
    end
end
...
2016.05.20.1  william motley  Rewrite entirely
2016.05.09.1  william motley  Bugfix nwuint32()
2016.04.26.1  william motley  Refactor string.byte functions
2013.11.06.1  william motley  Support string as well as payload
2013.02.22.1  william motley  Initial development - split from decodeASN()
...
function readASNlength(payload, position)
    ...
    local inType = type(payload)
    local length
    if inType == "string" then
        if #payload >= position then
            length = string.byte(payload, position)
            if length then
                position = position + 1
                if length > 128 then
                    ...
                    elseif lengthBytes == 2 then
                        length = struint16(payload, position)
                        if length then
                            position = position + 2
                        end
                    elseif lengthBytes == 3 then
                        length = struint24(payload, position)
                        if length then
                            position = position + 3
                        end
                    elseif lengthBytes == 4 then
                        length = struint32(payload, position)
                        if length then
                            position = position + 4
                        end
                    end
        ...
        length = nwpayload.uint8(payload, position)
        if length then
            position = position + 1
            if length > 128 then
                local lengthBytes = length - 128
                ...
                elseif lengthBytes == 2 then
                    length = nwpayload.uint16(payload, position)
                    if length then
                        position = position + 2
                    end
                elseif lengthBytes == 3 then
                    local tempByte1, tempByte2, tempByte3 = nwpayload.uint8(payload, position, position + 2)
                    if tempByte1 and tempByte2 and tempByte3 then
                        length = (((tempByte1 * 256) + tempByte2) * 256) + tempByte3
                        position = position + 3
                    end
                elseif lengthBytes == 4 then
                    length = nwpayload.uint32(payload, position)
                    if length then
                        position = position + 4
                    end
                end
    ...
    if not length or not position then
        length, position  = nil, nil
    end
...
2016.05.24.1d1                          Don't gsub unprintable characters
2016.05.20.1  william motley  Don't define function every time called
2016.05.12.1  william motley  In OID extraction, scope temp to local
2016.05.11.1  william motley  Preventative - wrap main in pcall
2016.05.09.1  william motley  Bugfix struint32()
2016.04.26.1  william motley  Refactor - convert to string before processing
2016.03.01.1  william motley  Don't continue looping buildASNconstructed() if length = 0
2013.11.06.1  william motley  Support string as well as payload
                                      Option to not strip non-printable characters from string types
2013.09.10.1  william motley  Remove 8 byte limit for integers
2013.09.09.1  william motley  Go back to returning a string for unknown simple type, but
                                      limit to 255 characters (before stripping unprintable)
2013.08.14.1  william motley  Use string.gsub, remove local function convertString()
...
2013.05.24.1  william motley  Make sure a value is returned before inserting into table
2013.05.06.1  william motley  Ensure hex values in an OID are 2 digits
2013.03.18.1  ahmed sonbol    Decode and return OID
2013.03.15.1  william motley  Return a hex representation of an int > 4 bytes
2013.03.07.1  william motley  Return simple type "null" as an empty string.
2013.02.22.1  william motley  Split readASNlength() into a separately exposed function
                                      Use table.insert()
2013.02.20.2  william motley  Added "type" index to tables and subtables
2013.02.17.1  william motley  Initial development
...
--]]
    local simpleASNtypes = ({
        ...
        [0x01] = -- boolean
            function(payload)
                return (payload and string.byte(payload) == 255 and true) or false
            end,
        [0x02] = -- integer
            function(payload)
                if not payload then
                ...
                elseif length == 2 then
                    do
                        return struint16(payload, 1)
                    end
                elseif length == 3 then
                    local tempByte1, tempByte2, tempByte3 = string.byte(payload, 1, 3)
                    if tempByte1 and tempByte2 and tempByte3 then
                        return (((tempByte1 * 256) + tempByte2) * 256) + tempByte3
                    end
                elseif length == 4 then
                    do
                        return struint32(payload, 1)
                    end
                else
                    -- Lua can't handle numbers bigger than 4 bytes, so return a hex representation instead.
                    local hexRep = ({ "0x" })
                    for i = 1, length do
                        table.insert(hexRep, string.format("%02x", string.byte(payload, i)))
                    end
        ...
        [0x04] = -- string
            function(payload)
                ...
                --return string.gsub(payload, "[^%w%p%s]", "")
                return payload
            end,
        [0x05] = -- null
            function(payload)
        ...
        [0x06] = -- oid
            function(payload)
                ...
                local numBytes = #payload
                if numBytes > 0 then
                    local byte = string.byte(payload, 1)
                    local OID, bigNumber = {}, {}
                    if byte then
                        ...
                        for position = 2, numBytes do
                            byte = string.byte(payload, position)
                            if byte then
                                ...
                                if byte < 128 then
                                    if #bigNumber == 0 then
                                        ...
                                        table.insert(bigNumber, byte)
                                        local temp = 0
                                        for i, j in ipairs(bigNumber) do
                                        	temp = bit.bor(bit.lshift(temp, 7), j)
                                        end
                                        table.insert(OID, temp)
                                        bigNumber = {}
                                    end
                                else
                                    table.insert(bigNumber, byte - 128)
                                end
                            else
                                break
                            end
    ...
    local function buildASNconstructed(payload)
        ...
        repeat
            ...
            local ASNtype = string.byte(payload, position)
            ...
            if ASNtype then
                ...
                if length and position then
                    ...
                    if bit.band(ASNtype, 32) == 32 then
                        ...
                        constructedTable = buildASNconstructed(string.sub(payload, position, position + length - 1))
                    ...
                    elseif simpleASNtypes[ASNtype] then
                        ...
                        local ASNpayload = string.sub(payload, position, position + length - 1)
                        ...
                        local ASNPayload = string.sub(payload, position, position + truncatedLength - 1)
...
function decodeASN(payload, position)
    if not payload then
        return
    end
    ...
    local ASNtype, length, ASNstring
    if type(payload) == "string" then
        ASNtype = string.byte(payload, position)
        if ASNtype then
            position = position + 1
            length, position = readASNlength(payload, position)
            if length and position then
                if #payload == length then
                    ...
                    ASNstring = string.sub(payload, position, position + length - 1)
                end
            end
        end
    else
        ASNtype = nwpayload.uint8(payload, position)
        if ASNtype then
            position = position + 1
            length, position = readASNlength(payload, position)
            if length and position then
                ASNstring = nwpayload.tostring(payload, position, position + length - 1)
            end
        end
    end
    if ASNtype and ASNstring then
        if bit.band(ASNtype, 32) == 32 then
            -- This is a constructed type: build a table with it and return the table.
            do
                local constructedTable = {}
                constructedTable = buildASNconstructed(ASNstring)
                constructedTable["type"] = ASNtype
            ...
            if simpleASNtypes[ASNtype] then
                ...
                ASNvalue = simpleASNtypes[ASNtype](ASNstring)
            else
                -- otherwise just return a string limited to 255 characters
                if #ASNstring > 255 then
                    return string.sub(ASNstring, 1, 255), position + length
                end
                return ASNstring, position + length
            end
        end
    end
end
...
2016.05.04.1  william motley  bugfix & refactor
2016.03.01.1  william motley  split from extractKerberos() and expanded
...
2016.03.01.1  william motley  split checkTable() into an independently exposed function
2015.05.12.1  william motley  remove pcalls from ASN table lookups
2013.12.04.1  william motley  bugfix: if type(ASN) ~= table -> if type(ASN) ~= "table"
2013.06.03.1  william motley  Error code is ASN[1][6][1] (not [1][5][1])
2013.05.23.1  william motley  Return error meta "unknown" for unknown error codes
2013.04.26.1  william motley  Wrap ASN table lookups in pcall
2013.04.25.1  william motley  Bugfix - reference K5cryptoTypes lookup table for crypto meta extraction
2013.03.06.1  william motley  Initial development
...
function extractKerberos(payload)
    ...
    local K5messageTypes = ({
        [0x0A] = -- AS REQ (10)
            function(ASN)
                local metaValues = ({[1] = ({["action"] = "kerberos as request"})})
                if checkTable(ASN, "[1][4][1][2][1][2][1][1]")
                and type(ASN[1][4][1][2][1][2][1][1]) == "string" then
                    table.insert(metaValues, ({["ad.username.src"] = ASN[1][4][1][2][1][2][1][1]}))
                end
                if checkTable(ASN, "[1][4][1][9][1][1][2][1]")
                and type(ASN[1][4][1][9][1][1][2][1]) == "string" then
                    table.insert(metaValues, ({["ad.computer.src"] = ASN[1][4][1][9][1][1][2][1]}))
                end
                if checkTable(ASN, "[1][4][1][3][1]")
                and type(ASN[1][4][1][3][1]) == "string" then
                    table.insert(metaValues, ({["ad.domain.src"]  = ASN[1][4][1][3][1]}))
                end
                if checkTable(ASN, "[1][4][1][4][1][2][1][1]")
                and type(ASN[1][4][1][4][1][2][1][1]) == "string" then
                    table.insert(metaValues, ({["ad.computer.dst"] = ASN[1][4][1][4][1][2][1][1]}))
                end
                if checkTable(ASN, "[1][4][1][4][1][2][1][2]")
                and type(ASN[1][4][1][4][1][2][1][2]) == "string" then
                    table.insert(metaValues, ({["ad.domain.dst"] = ASN[1][4][1][4][1][2][1][2]}))
                end
                if checkTable(ASN, "[1][4][1][8][1]")
                and type(ASN[1][4][1][8][1]) == "table" then
                    for idx,cryptoVal in ipairs(ASN[1][4][1][8][1]) do
                        if type(cryptoVal) == "number"
                        and K5cryptoTypes[cryptoVal] then
                            table.insert(metaValues, ({["crypto"] = K5cryptoTypes[cryptoVal] }))
                        end
                    end
                end
                return metaValues
        ...
        [0x0B] = -- AS REP (11)
            function(ASN)
                local metaValues = ({[1] = ({["action"] = "kerberos as reply" })})
                if checkTable(ASN, "[1][3][1]")
                and type(ASN[1][3][1]) == "string" then
                    table.insert(metaValues, ({["ad.domain.src"] = ASN[1][3][1]}))
                end
                if checkTable(ASN, "[1][4][1][2][1][1]")
                and type(ASN[1][4][1][2][1][1]) == "string" then
                    table.insert(metaValues, ({["ad.username.src"] = ASN[1][4][1][2][1][1]}))
                end
                if checkTable(ASN, "[1][5][1][1][2][1]")
                and type(ASN[1][5][1][1][2][1]) == "string" then
                    table.insert(metaValues, ({["ad.domain.dst"] = ASN[1][5][1][1][2][1]}))
                end
                if checkTable(ASN, "[1][5][1][1][4][1][1][1]")
                and type(ASN[1][5][1][1][4][1][1][1]) == "number"
                and K5cryptoTypes[ASN[1][5][1][1][4][1][1][1]] then
                    table.insert(metaValues, ({["crypto"] = K5cryptoTypes[ASN[1][5][1][1][4][1][1][1]]}))
                end
                if checkTable(ASN, "[1][6][1][1][1]")
                and type(ASN[1][6][1][1][1]) == "number"
                and K5cryptoTypes[ASN[1][6][1][1][1]] then
                    table.insert(metaValues, ({["crypto"] = K5cryptoTypes[ASN[1][6][1][1][1]]}))
                end
                if checkTable(ASN, "[1][5][1][1][3][1][2][1]")
                and type(ASN[1][5][1][1][3][1][2][1]) == "table" then
                    local numNames = #ASN[1][5][1][1][3][1][2][1]
                    for idx, name in ipairs(ASN[1][5][1][1][3][1][2][1]) do
                        if type(name) == "string" then
                            if idx ~= numNames then
                                table.insert(metaValues, ({["ad.computer.dst"] = name }))
                            else
                                table.insert(metaValues, ({["ad.domain.dst"] = name }))
                            end
                        end
                    end
                end
        ...
        [0x0C] = -- TGS REQ (12)
            function(ASN)
                local metaValues = ({[1] = ({ ["action"] = "kerberos tgs request" })})
                if checkTable(ASN, "[1][4][1][2][1]")
                and type(ASN[1][4][1][2][1]) == "string" then
                    table.insert(metaValues, ({["ad.domain.src"] = ASN[1][4][1][2][1] }))
                end
                if checkTable(ASN, "[1][4][1][3][1][2][1]")
                and type(ASN[1][4][1][3][1][2][1]) == "table" then
                    for idx, name in ipairs(ASN[1][4][1][3][1][2][1]) do
                        if type(name) == "string" then
                            table.insert(metaValues, ({["ad.computer.dst"] = name }))
                        end
                    end
                end
                if checkTable(ASN, "[1][4][1][6][1]")
                and type(ASN[1][4][1][6][1]) == "table" then
                    for idx, crypto in ipairs(ASN[1][4][1][6][1]) do
                        if type(crypto) == "number"
                        and K5cryptoTypes[crypto] then
                            table.insert(metaValues, ({["crypto"] = K5cryptoTypes[crypto] }))
                        end
                    end
                end
                return metaValues
        ...
        [0x0D] = -- TGS REP (13)
            function(ASN)
                local metaValues = ({[1] = ({ ["action"] = "kerberos tgs reply" })})
                if checkTable(ASN, "[1][3][1]")
                and type(ASN[1][3][1]) == "string" then
                    table.insert(metaValues, ({["ad.domain.src"] = ASN[1][3][1]}))
                end
                if checkTable(ASN, "[1][4][1][2][1][1]")
                and type(ASN[1][4][1][2][1][1]) == "string" then
                    table.insert(metaValues, ({["ad.computer.src"] = ASN[1][4][1][2][1][1]}))
                end
                if checkTable(ASN, "[1][5][1][1][3][1][2][1][1]")
                and type(ASN[1][5][1][1][3][1][2][1][1]) == "string" then
                    table.insert(metaValues, ({["ad.computer.dst"] = ASN[1][5][1][1][3][1][2][1][1]}))
                end
                if checkTable(ASN, "[1][5][1][1][4][1][1][1]")
                and type(ASN[1][5][1][1][4][1][1][1]) == "number"
                and K5cryptoTypes[ASN[1][5][1][1][4][1][1][1]] then
        ...
        [0x0E] = -- AP REQ (14)
            function(ASN)
                local metaValues = ({[1] = ({ ["action"] = "kerberos ap request" })})
                if checkTable(ASN, "[1][4][1][1][2][1]")
                and type(ASN[1][4][1][1][2][1]) == "string" then
                    table.insert(metaValues, ({["ad.domain.src"] = ASN[1][4][1][1][2][1]}))
                end
                if checkTable(ASN, "[1][4][1][1][3][1][2][1]")
                and type(ASN[1][4][1][1][3][1][2][1]) == "table" then
                    local numNames = #ASN[1][4][1][1][3][1][2][1]
                    for idx, name in ipairs(ASN[1][4][1][1][3][1][2][1]) do
                        if type(name) == "string" then
                            if idx ~= numNames then
                                table.insert(metaValues, ({["ad.computer.dst"] = name }))
                            else
                ...
                if checkTable(ASN, "[1][4][1][1][4][1][1][1]")
                and type(ASN[1][4][1][1][4][1][1][1]) == "number"
                and K5cryptoTypes[ASN[1][4][1][1][4][1][1][1]] then
                    table.insert(metaValues, ({["crypto"] = K5cryptoTypes[ASN[1][4][1][1][4][1][1][1]]}))
                end
                if checkTable(ASN, "[1][5][1][1][1]")
                and type(ASN[1][5][1][1][1]) == "number"
                and K5cryptoTypes[ASN[1][5][1][1][1]] then
                    table.insert(metaValues, ({["crypto"] = K5cryptoTypes[ASN[1][5][1][1][1]]}))
                end
                return metaValues
            end,
        [0x0F] = -- AP REP (15)
        ...
        [0x1E] = -- ERROR (30)
            function(ASN)
                local metaValues = {}
                if checkTable(ASN, "[1][6][1]")
                and type(ASN[1][6][1]) == "number" then
                    if K5errorCodes[ASN[1][6][1]] then
                        table.insert(metaValues, ({["error"] = K5errorCodes[ASN[1][6][1]]}))
                    else
                        table.insert(metaValues, ({["error"] = "unknown"}))
                    end
                end
                if checkTable(ASN, "[1][7][1][2][1][1]")
                and type(ASN[1][7][1][2][1][1]) == "string" then
                    table.insert(metaValues, ({["ad.computer.dst"] = ASN[1][7][1][2][1][1] }))
                end
                if checkTable(ASN, "[1][7][1][2][1][2]")
                and type(ASN[1][7][1][2][1][2]) == "string" then
                    table.insert(metaValues, ({["ad.domain.dst"] = ASN[1][7][1][2][1][2] }))
                end
                return metaValues
            end,
    })
    ...
    if checkTable(ASN, "[1][2][1]") then
        local messageType = ASN[1][2][1]
        if type(messageType) == "number"
        and K5messageTypes[messageType] then
            local K5meta = K5messageTypes[messageType](ASN)
            return K5meta
        end
    end
end
...
2016.05.12.1  william motley  bugfix scope certTable to local
2016.05.10.1  william motley  Initial development

Expects a string or payload object containing an x509 certificate.

The first byte of the string or payload object must be the first
byte of the ASN.1 structure of the certificate.

The string or payload object must contain the entire certificate,
though it does not need to consist exclusively of the certificate.

Returns a lua table:

    {
        ["serial"] = "0x...",       -- certificate serial number represented as hex (string)
        ["issuer"] = {
            ["orgName"] = "..."     -- issuer organizational name
            ["commonName"] = "..."  -- issuer common name
        },
        ["subject"] = {
            ["orgName"] = "..."     -- subject organizational name
            ["commonName"] = "..."  -- subject common name
        }
    }
    
WARNING:  Not all of the elements described above may be present.
          Check before registering meta.
          
          local certValues = nwll.x509(payload:sub(x,y))
          if certValues then
            if certValues.serial then
                nw.createMeta(self.keys["ssl.serial"], certValues.serial
            end
            if certValues.issuer then
                if certValues.issuer.orgName then
                    nw.createMeta(self.keys["ssl.ca"], certValues.issuer.orgName
                end
                ...
            ...
          end
...
function x509(certificate)
    ...
    local certTable = decodeASN(certificate)
    if not certTable or type(certTable) ~= "table" then
        return
    end
    local certValues = {["issuer"] = {}, ["subject"] = {}}
    local serial, issuer, subject
    if certTable[1] and certTable[1][1] then
        if type(certTable[1][1]) == "number" and certTable[1][1] == 2  then
            serial = certTable[2]
            issuer = certTable[4]
            subject = certTable[6]
        elseif type(certTable[1][1]) == "string" then
            serial = certTable[1][1]
            issuer = certTable[1][3]
            subject = certTable[1][5]
        elseif type(certTable[1][1]) == "table" and certTable[1][1][1] and type(certTable[1][1][1]) == "number" and certTable[1][1][1] == 2 then
            serial = certTable[1][2]
            issuer = certTable[1][4]
            subject = certTable[1][6]
        end
    end
    if serial and type(serial) ~= "table" and issuer and type(issuer) == "table" and subject and type(subject) == "table" then
        if type(serial) == "string" then
            certValues.serial = serial
        elseif type(serial) == "number" then
            -- convert to hex for consistency
            certValues.serial = ("0x" .. string.match(bit.tohex(serial), "^0-(%x+)"))
        else
            return
        end
        for RDN_element_num,RDN_element in ipairs(issuer) do
            if type(RDN_element) == "table" and checkTable(RDN_element, "[1][1]") then
                if RDN_element[1][1] == "2.5.4.10" and RDN_element[1][2] and type(RDN_element[1][2]) == "string" then
                    certValues.issuer.orgName = RDN_element[1][2]
                elseif RDN_element[1][1] == "2.5.4.3" and RDN_element[1][2] and type(RDN_element[1][2]) == "string" then
                    certValues.issuer.commonName = RDN_element[1][2]
                end
                if certValues.issuer.orgName and certValues.issuer.commonName then
                    break
                end
            end
        end
        for RDN_element_num,RDN_element in ipairs(subject) do
            if type(RDN_element) == "table" and checkTable(RDN_element, "[1][1]") then
                if RDN_element[1][1] == "2.5.4.10" and RDN_element[1][2] and type(RDN_element[1][2]) == "string" then
                    certValues.subject.orgName = RDN_element[1][2]
                elseif RDN_element[1][1] == "2.5.4.3" and RDN_element[1][2] and type(RDN_element[1][2]) == "string" then
                    certValues.subject.commonName = RDN_element[1][2]
                end
                if certValues.subject.orgName and certValues.subject.commonName then
                    break
                end
            end
        end
...
2015.09.01.1  william motley  Convert payload to string
2014.03.21.1  william motley  Support decoding strings (not just payload)
2013.04.19.1  william motley  Initial development
...
2013.04.22.1  william motley  Initial development
...
2013.05.31.1  william motley  Converted to big endian
2013.03.04.1  william motley  Moved from smb.lua
...
2016.05.20.1  william motley  don't define OIDTable every time called
2016.05.12.1  william motley  bugfix: lostDot -> lastDot
2013.08.22.1  william motley  Moved from nwsnmp.lua
...
--return parserName, parserVersion, parserDetails, conflicts, dependencies, keyUsage, liveTags